...
# Define pages
load_data_page = st.Page("app_pages/load_data.py", title="載入資料", icon=":material/upload_file:")
...
# Store load_data_page so utils.py can call st.switch_page(page_obj) correctly
st.session_state["_pages"] = {"load_data": load_data_page}

page = st.navigation(
    {
        # 空字串群組：頁面必須在 navigation 中註冊才能被 st.switch_page 切換，
        # 但透過下方 CSS 將此群組與其項目完全隱藏，使用者不會看到它。
        "": [load_data_page],
...
# 隱藏「載入資料」頁面的 sidebar 項目（第一個無標題群組整組隱藏）
st.markdown(
    """
    <style>
    /* 隱藏 navigation 中第一個空群組（載入資料）*/
